...
function onLockChanged() onStateChanged(); end
...
function onStateChanged()
    if header.subwindow then header.subwindow.update(); end
    if main.subwindow then main.subwindow.update(); end
end
...
function onNameUpdated()
    ...
    if bID then
        ...
        if sTooltip == "" then
            sTooltip =
                Interface.getString("library_recordtype_empty_nonid_item")
        end
    end
